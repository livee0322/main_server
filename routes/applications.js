...
router.get('/mine',
  ...
  async (req, res) => {
    ...
    if (!mongoose.isValidObjectId(campaignId)) return res.fail('INVALID_ID','INVALID_ID',400);
...
router.post('/',
  ...
  async (req, res) => {
    ...
    if (c.type !== 'recruit') return res.fail('BAD_REQUEST','NOT_RECRUIT',400);
    ...
    try {
      const created = await Application.create({
        campaignId, userId: req.user.id, profileRef, message
      });
      return res.ok({ data: toDTO(created) }, 201);
    } catch (e) {
      if (e?.code === 11000) return res.fail('DUPLICATE','ALREADY_APPLIED',409);
      throw e;
    }
  }
);
...
router.get('/',
  ...
  async (req, res) => {
    ...
    const items = await Application.find({ campaignId }).sort({ createdAt: -1 });
