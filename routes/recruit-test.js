...
const ok   = (res, payload, status=200)=> (typeof res.ok==='function'?res.ok(payload,status):res.status(status).json(payload));
...
function pickBrand(o={}){
  ...
  const v=c.find(x=>x!=null && String(x).trim()!==''); return v?String(v).trim():'';
}
function parsePay(o={}){
...
router.post('/',
  ...
  async (req,res)=>{
    ...
    try{
      ...
      // 썸네일 자동 보정
      if(payload.coverImageUrl && !payload.thumbnailUrl) payload.thumbnailUrl=toThumb169(payload.coverImageUrl);

      // 날짜 보정
      if(payload.recruit?.shootDate){ const d=new Date(payload.recruit.shootDate); if(!isNaN(d)) payload.shootDate=d; }
...
router.put('/:id', auth, requireRole('brand','admin','showhost'), async (req,res)=>{
  ...
  try{
    ...
    if(brandName){ $set.brandName=brandName; $set.recruit={...($set.recruit||{}),brandName}; }

    if($set.coverImageUrl && !$set.thumbnailUrl) $set.thumbnailUrl=toThumb169($set.coverImageUrl);
    if($set.recruit?.shootDate){ const d=new Date($set.recruit.shootDate); if(!isNaN(d)) $set.shootDate=d; }
    ...
    const updated=await Recruit.findOneAndUpdate({ _id:id, createdBy:req.user.id },{ $set },{ new:true });
